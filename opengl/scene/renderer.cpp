...
namespace scene {
    ...
    void Renderer::MSAA(bool enable) {
        ...
        if (samples == 0) {
            (*gl)->glGetIntegerv(GL_SAMPLE_BUFFERS, &buffers);
    ...
    void Renderer::StencilTest(bool enable) {
        ...
        if (enable && !is_enabled) {
            ...
            (*gl)->glStencilFunc(GL_EQUAL, 1, 0xFF);  // discard fragments whose stencil values != 1
            is_enabled = true;
        }
        else if (!enable && is_enabled) {
    ...
    void Renderer::SeamlessCubemap(bool enable) {
        ...
        if (enable && !is_enabled) {
            ...
            (*gl)->glDisable(GL_TEXTURE_CUBE_MAP_SEAMLESS);
            is_enabled = false;
        }
    }
    ...
    void Renderer::PrimitiveRestart(bool enable) {
        ...
        if (enable && !is_enabled) {
            ...
            (*gl)->glPrimitiveRestartIndex(0xFFFFFF);
            is_enabled = true;
        }
        else if (!enable && is_enabled) {
    ...
    void Renderer::SetFrontFace(bool ccw) {
        (*gl)->glFrontFace(ccw ? GL_CCW : GL_CW);
    }
    ...
    void Renderer::Reset() {
        ...
        PrimitiveRestart(0);
        SetFrontFace(1);
        auto pointer=reinterpret_cast<QOpenGLWidget*>(*gl);
        SetViewport(pointer->width(),pointer->height());
        SetShadowPass(0);
    }
    ...
    void Renderer::Clear() {
        ...
        (*gl)->glClearStencil(0);  // 8-bit integer
    ...
    void Renderer::Render(const asset_ref<asset::Shader> custom_shader) {
        ...
        if (!render_queue.empty()) {
            ...
            auto pointer=reinterpret_cast<QOpenGLWidget*>((*gl));
            glm::ivec2 resolution = {pointer->width(),pointer->height()};

...
bool Sync::Signaled() {
    ...
    */
    ...
    (*gl)->glGetSynciv(sync, GL_SYNC_STATUS, sizeof(GLint), NULL, &status);
...
void Sync::ClientWaitSync(GLuint64 timeout) {
    ...
    GLenum status = (*gl)->glClientWaitSync(sync, GL_SYNC_FLUSH_COMMANDS_BIT, timeout);
    ...
    while (status == GL_TIMEOUT_EXPIRED) {
        status = (*gl)->glClientWaitSync(sync, 0, timeout);  // subsequent calls don't need the flush bit
...
GLint64 Sync::GetServerTimeout() {
    ...
    if (max_server_timeout < 0) {
        (*gl)->glGetInteger64v(GL_MAX_SERVER_WAIT_TIMEOUT, &max_server_timeout);
    }

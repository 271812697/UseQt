...
namespace component {
    ...
    void Uniform<T>::Upload(T val, GLuint index) const {
        ...
        /**/ if constexpr (std::is_same_v<T, bool>)   {(*gl)-> glProgramUniform1i(id, lc, static_cast<int>(val)); }
        else if constexpr (std::is_same_v<T, int>)    { (*gl)->glProgramUniform1i(id, lc, val); }
        else if constexpr (std::is_same_v<T, float>)  {(*gl)-> glProgramUniform1f(id, lc, val); }
        else if constexpr (std::is_same_v<T, GLuint>) { (*gl)->glProgramUniform1ui(id, lc, val); }
        else if constexpr (std::is_same_v<T, vec2>)   { (*gl)->glProgramUniform2fv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, vec3>)   { (*gl)->glProgramUniform3fv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, vec4>)   { (*gl)->glProgramUniform4fv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, ivec2>)  { (*gl)->glProgramUniform2iv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, ivec3>)  { (*gl)->glProgramUniform3iv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, ivec4>)  { (*gl)->glProgramUniform4iv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, uvec2>)  { (*gl)->glProgramUniform2uiv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, uvec3>)  { (*gl)->glProgramUniform3uiv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, uvec4>)  { (*gl)->glProgramUniform4uiv(id, lc, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, mat2>)   { (*gl)->glProgramUniformMatrix2fv(id, lc, 1, GL_FALSE, &val[0][0]); }
        else if constexpr (std::is_same_v<T, mat3>)   { (*gl)->glProgramUniformMatrix3fv(id, lc, 1, GL_FALSE, &val[0][0]); }
        else if constexpr (std::is_same_v<T, mat4>)   { (*gl)->glProgramUniformMatrix4fv(id, lc, 1, GL_FALSE, &val[0][0]); }
        else {
    ...
    void Material::SetShader(asset_ref<Shader> shader_ref) {
        ...
        GLint n_uniforms;
        (*gl)->glGetProgramInterfaceiv(id, GL_UNIFORM, GL_ACTIVE_RESOURCES, &n_uniforms);

        GLenum meta_data[] = { GL_NAME_LENGTH, GL_TYPE, GL_LOCATION, GL_BLOCK_INDEX };

        for (int i = 0; i < n_uniforms; ++i) {
            GLint unif_info[4] {};
            (*gl)->glGetProgramResourceiv(id, GL_UNIFORM, i, 4, meta_data, 4, NULL, unif_info);
           ...
           (*gl)->glGetProgramResourceName(id, GL_UNIFORM, i, name_length, NULL, name);

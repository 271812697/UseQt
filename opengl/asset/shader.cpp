...
namespace asset {
    ...
    Shader::Shader(const std::string& binary_path, GLenum format) : IAsset(), source_path() {
        ...
        GLuint program_id = (*gl)->glCreateProgram();
        (*gl)->glProgramBinary(program_id, format, buffer.data(), buffer.size());

        GLint status;
        (*gl)->glGetProgramiv(program_id, GL_LINK_STATUS, &status);
        ...
        if (status == GL_FALSE) {
            ...
            (*gl)->glGetProgramInfoLog(program_id, info_log_length, NULL, &info_log[0]);
    ...
    void Shader::Save() const {
        ...
        GLint binary_length;
        (*gl)->glGetProgramiv(id, GL_PROGRAM_BINARY_LENGTH, &binary_length);
        CORE_TRACE("Retrieving shader binary length ... : {0}", binary_length);

        GLenum binary_format;
        std::vector<GLubyte> buffer(binary_length);
        (*gl)->glGetProgramBinary(id, binary_length, NULL, &binary_format, buffer.data());
    ...
    void Shader::SetUniform(GLuint location, const T& val) const {
        ...
        /**/ if constexpr (std::is_same_v<T, bool>)   { (*gl)->glProgramUniform1i(id, location, static_cast<int>(val)); }
        else if constexpr (std::is_same_v<T, int>)    { (*gl)->glProgramUniform1i(id, location, val); }
        else if constexpr (std::is_same_v<T, float>)  { (*gl)->glProgramUniform1f(id, location, val); }
        else if constexpr (std::is_same_v<T, GLuint>) { (*gl)->glProgramUniform1ui(id, location, val); }
        else if constexpr (std::is_same_v<T, vec2>)   { (*gl)->glProgramUniform2fv(id, location, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, vec3>)   { (*gl)->glProgramUniform3fv(id, location, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, vec4>)   { (*gl)->glProgramUniform4fv(id, location, 1, &val[0]); }
        else if constexpr (std::is_same_v<T, mat2>)   { (*gl)->glProgramUniformMatrix2fv(id, location, 1, GL_FALSE, &val[0][0]); }
        else if constexpr (std::is_same_v<T, mat3>)   { (*gl)->glProgramUniformMatrix3fv(id, location, 1, GL_FALSE, &val[0][0]); }
        else if constexpr (std::is_same_v<T, mat4>)   { (*gl)->glProgramUniformMatrix4fv(id, location, 1, GL_FALSE, &val[0][0]); }
        else if constexpr (std::is_same_v<T,QMatrix4x4>){

            (*gl)->glProgramUniformMatrix4fv(id, location, 1, GL_FALSE, val.data());
    ...
    void Shader::ReadShader(const std::string& path, const std::string& macro, std::string& output) {
        ...
        if (!file_stream.is_open()) {
             CORE_ERROR(("Unable to read shader file {0} ... ", path).c_str());
            return;
        }
    ...
    void Shader::LoadShader(GLenum type) {
        ...
        GLuint shader_id = (*gl)->glCreateShader(type);
        (*gl)->glShaderSource(shader_id, 1, &c_source_code, nullptr);
        (*gl)->glCompileShader(shader_id);

        GLint status;
        (*gl)->glGetShaderiv(shader_id, GL_COMPILE_STATUS, &status);

        if (status == GL_FALSE) {
            GLint info_log_length;
            (*gl)->glGetShaderiv(shader_id, GL_INFO_LOG_LENGTH, &info_log_length);

            GLchar* info_log = new GLchar[info_log_length + 1];
            (*gl)->glGetShaderInfoLog(shader_id, info_log_length, NULL, info_log);

            CORE_ERROR("Failed to compile shader: {0}", info_log);
            delete[] info_log;
            (*gl)->glDeleteShader(shader_id);  // prevent shader leak
    ...
    void Shader::LinkShaders() {
        ...
        GLint status;
        (*gl)->glGetProgramiv(pid, GL_LINK_STATUS, &status);

        if (status == GL_FALSE) {
            GLint info_log_length;
            (*gl)->glGetProgramiv(pid, GL_INFO_LOG_LENGTH, &info_log_length);

            GLchar* info_log = new GLchar[info_log_length + 1];
            (*gl)->glGetProgramInfoLog(pid, info_log_length, NULL, info_log);
    ...
    CShader::CShader(const std::string& source_path) : Shader() {
        ...
        GLint local_size[3];
        (*gl)->glGetProgramiv(id, GL_COMPUTE_WORK_GROUP_SIZE, local_size);
    ...
    // this is the `catch` block in the initializer list, not the ctor body
        ...
        CORE_ERROR("Cannot load compute shader: {0}");
    ...
    void CShader::Dispatch(GLuint nx, GLuint ny, GLuint nz) const {
        ...
        static GLuint max_invocations = GetAppInstance().cs_max_invocations;
        //CORE_ASERT(local_size_x * local_size_y * local_size_z <= max_invocations, "Compute size overflow!");

        static GLuint cs_sx = GetAppInstance().cs_sx;
        static GLuint cs_sy = GetAppInstance().cs_sy;
        static GLuint cs_sz = GetAppInstance().cs_sz;
        ...
        static GLuint cs_nx = GetAppInstance().cs_nx;
        static GLuint cs_ny = GetAppInstance().cs_ny;
        static GLuint cs_nz = GetAppInstance().cs_nz;
...
*/
    ...
    void CShader::SyncWait(GLbitfield barriers) const {
        (*gl)->glMemoryBarrier(barriers);  // sync to ensure all writes are complete
    }

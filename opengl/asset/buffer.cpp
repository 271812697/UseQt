...
namespace asset {
	...
	IBuffer::IBuffer(GLsizeiptr size, const void* data, GLbitfield access) : size(size), data_ptr() {
		...
		(*gl)->glNamedBufferStorage(id, size, data, access);  // immutable storage
	}

	IBuffer::~IBuffer() {
		CORE_INFO("DeletBuffer:{}", id);
		//(*gl)->glDeleteBuffers(1, &id);  // bound buffers will be unbound, `id = 0` will be ignored
	}
	...
	void IBuffer::GetData(void* data) const {
		(*gl)->glGetNamedBufferSubData(id, 0, this->size, data);
	}
	...
	void IBuffer::SetData(const void* data) const {
		(*gl)->glNamedBufferSubData(id, 0, this->size, data);
	}
	...
	void IBuffer::Acquire(GLbitfield access) const {
		if (!this->data_ptr) {
			if (data_ptr = (*gl)->glMapNamedBufferRange(id, 0, this->size, access); !data_ptr) {
	...
	void IBuffer::Release() const {
		if (this->data_ptr) {
			...
			if (GLboolean ret = (*gl)->glUnmapNamedBuffer(id); ret == GL_FALSE) {
	...
	void IBuffer::Clear() const {
		(*gl)->glClearNamedBufferSubData(id, GL_R8UI, 0, this->size, GL_RED, GL_UNSIGNED_BYTE, NULL);
	}

	void IBuffer::Clear(GLintptr offset, GLsizeiptr size) const {
		(*gl)->glClearNamedBufferSubData(id, GL_R8UI, offset, size, GL_RED, GL_UNSIGNED_BYTE, NULL);
	}

	void IBuffer::Flush() const {
		(*gl)->glFlushMappedNamedBufferRange(id, 0, this->size);
	}

	void IBuffer::Flush(GLintptr offset, GLsizeiptr size) const {
		(*gl)->glFlushMappedNamedBufferRange(id, offset, size);
	}

	void IBuffer::Invalidate() const {
		(*gl)->glInvalidateBufferSubData(id, 0, this->size);
	}
	...
	void IBuffer::Copy(GLuint fr, GLuint to, GLintptr fr_offset, GLintptr to_offset, GLsizeiptr size) {
		(*gl)->glCopyNamedBufferSubData(fr, to, fr_offset, to_offset, size);
	}

	void IIndexedBuffer::Reset(GLuint index) {
		this->index = index;
		(*gl)->glBindBufferBase(this->target, index, id);
	}
		...
		: IIndexedBuffer(), offset_vec(offset), length_vec(length), stride_vec(stride)
	{
		...
		(*gl)->glCreateBuffers(1, &id);
		(*gl)->glNamedBufferStorage(id, this->size, NULL, GL_DYNAMIC_STORAGE_BIT);
		(*gl)->glBindBufferBase(this->target, this->index, id);
	}
	...
	UBO::UBO(GLuint shader, GLuint block_id, GLbitfield access) : IIndexedBuffer() {
		...
		GLint block_info[4];
		(*gl)->glGetProgramResourceiv(shader, GL_UNIFORM_BLOCK, block_id, 4, props1, 4, NULL, block_info);
		...
		(*gl)->glGetProgramResourceName(shader, GL_UNIFORM_BLOCK, block_id, block_info[1], NULL, name);
		std::string block_name(name);  // uniform block name
		delete[] name;

		GLint n_unifs = block_info[0];
		GLint* indices = new GLint[n_unifs];
		(*gl)->glGetProgramResourceiv(shader, GL_UNIFORM_BLOCK, block_id, 1, props2, n_unifs, NULL, indices);
		...
		for (auto& u_index : unif_indices) {
			GLint unif_info[5];
			(*gl)->glGetProgramResourceiv(shader, GL_UNIFORM, u_index, 5, unif_props, 5, NULL, unif_info);

			char* name = new char[unif_info[0]];
			(*gl)->glGetProgramResourceName(shader, GL_UNIFORM, u_index, unif_info[0], NULL, name);
		...
		(*gl)->glCreateBuffers(1, &id);
		(*gl)->glNamedBufferStorage(id, this->size, NULL, access);
		(*gl)->glBindBufferBase(this->target, this->index, id);
	}

	void UBO::SetUniform(GLuint uid, const void* data) const {
		(*gl)->glNamedBufferSubData(id, offset_vec[uid], length_vec[uid], data);  // update a single uniform
	}
	...
	void UBO::SetUniform(GLuint fr, GLuint to, const void* data) const {
		...
		(*gl)->glNamedBufferSubData(id, offset_vec[fr], n_bytes, data);  // update a range of uniforms
	}

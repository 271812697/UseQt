...
namespace asset {
	...
	RBO::RBO(GLuint width, GLuint height, bool multisample) : IAsset() {
		...
		if (multisample) {
			(*gl)->glNamedRenderbufferStorageMultisample(id, 4, GL_DEPTH24_STENCIL8, width, height);
		}
		else {
			(*gl)->glNamedRenderbufferStorage(id, GL_DEPTH24_STENCIL8, width, height);
		}
	}
	...
	FBO::FBO(GLuint width, GLuint height) : IAsset(), width(width), height(height), status(0) {
		(*gl)->glDisable(GL_FRAMEBUFFER_SRGB);  // important! turn off colorspace correction globally
		...
		if (!internal_shader) {
			internal_shader = WrapAsset<Shader>(ASSET_DIR + std::string("framebuffer.glsl"));
		}
	}
	...
	void FBO::AddColorTexture(GLuint count, bool multisample) {
		...
		for (GLuint i = 0; i < count; i++) {
			...
			if (!multisample) {
				(*gl)->glTextureParameteri(tid, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
				(*gl)->glTextureParameteri(tid, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
				(*gl)->glTextureParameteri(tid, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
				(*gl)->glTextureParameteri(tid, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
				(*gl)->glTextureParameterfv(tid, GL_TEXTURE_BORDER_COLOR, border);
			}

			(*gl)->glNamedFramebufferTexture(id, GL_COLOR_ATTACHMENT0 + n_color_buffs + i, tid, 0);
		}
	...
	void FBO::SetColorTexture(GLenum index, GLuint texture_2d) {
		...
		(*gl)->glNamedFramebufferTexture(id, GL_COLOR_ATTACHMENT0 + index, texture_2d, 0);
	...
	void FBO::SetColorTexture(GLenum index, GLuint texture_cubemap, GLuint face) {
		...
		if constexpr (true) {
			...
			(*gl)->glNamedFramebufferTextureLayer(id, GL_COLOR_ATTACHMENT0 + index, texture_cubemap, 0, face);
		}
		else {
			(*gl)->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, texture_cubemap, 0);
		}
	...
	void FBO::AddDepStTexture(bool multisample) {
		...
		// stencil texture for MSAA because filtering on multisampled textures is not allowed by Open(*gl)->
		if (multisample) {
		...
		(*gl)->glTextureParameteri(depst_texture->ID(), GL_DEPTH_STENCIL_TEXTURE_MODE, GL_DEPTH_COMPONENT);

		GLint immutable_format;
		(*gl)->glGetTextureParameteriv(depst_texture->ID(), GL_TEXTURE_IMMUTABLE_FORMAT, &immutable_format);
		...
		(*gl)->glTextureParameteri(stencil_view->ID(), GL_DEPTH_STENCIL_TEXTURE_MODE, GL_STENCIL_INDEX);

		(*gl)->glNamedFramebufferTexture(id, GL_DEPTH_STENCIL_ATTACHMENT, depst_texture->ID(), 0);
	...
	void FBO::AddDepStRenderBuffer(bool multisample) {
		...
		(*gl)->glNamedFramebufferRenderbuffer(id, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, depst_renderbuffer->ID());
	...
	void FBO::AddDepthCubemap() {
		...
		(*gl)->glTextureParameteri(tid, GL_DEPTH_STENCIL_TEXTURE_MODE, GL_DEPTH_COMPONENT);
		(*gl)->glTextureParameteri(tid, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		(*gl)->glTextureParameteri(tid, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		(*gl)->glTextureParameteri(tid, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		(*gl)->glTextureParameteri(tid, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		(*gl)->glTextureParameteri(tid, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);

		(*gl)->glNamedFramebufferTexture(id, GL_DEPTH_ATTACHMENT, tid, 0);
		const GLenum null[] = { GL_NONE };
		(*gl)->glNamedFramebufferReadBuffer(id, GL_NONE);
		(*gl)->glNamedFramebufferDrawBuffers(id, 1, null);
	...
	void FBO::SetDrawBuffer(GLuint index) const {
		...
		(*gl)->glNamedFramebufferDrawBuffers(id, 1, buffers);
	}
	...
	void FBO::SetDrawBuffers(std::vector<GLuint> indices) const {
		...
		(*gl)->glNamedFramebufferDrawBuffers(id, n_index, buffers);
		delete[] buffers;
	}
	...
	void FBO::SetDrawBuffers() const {
		...
		if (size_t n = color_attachments.size(); n > 0) {
			...
			(*gl)->glNamedFramebufferDrawBuffers(id, n, attachments);
			delete[] attachments;
		}
	}
	...
	void FBO::Draw(GLint index) const {
		...
		(*gl)->glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, 1, &subroutine_index);

		(*gl)->glDrawArrays(GL_TRIANGLES, 0, 3);  // bufferless quad rendering
	}
	...
	void FBO::Clear(GLint index) const {
		...
		if (index >= 0) {
			(*gl)->glClearNamedFramebufferfv(id, GL_COLOR, index, clear_color);
		}
		// clear the depth buffer
		else if (index == -1) {
			(*gl)->glClearNamedFramebufferfv(id, GL_DEPTH, 0, &clear_depth);
		}
		// clear the stencil buffer
		else if (index == -2) {
			(*gl)->glClearNamedFramebufferiv(id, GL_STENCIL, 0, &clear_stencil);
		}
		else {
			CORE_ERROR("Buffer index {0} is not valid in the framebuffer!", index);
	...
	void FBO::CopyColor(const FBO& fr, GLuint fr_idx, const FBO& to, GLuint to_idx) {
		...
		(*gl)->glNamedFramebufferReadBuffer(fr.id, GL_COLOR_ATTACHMENT0 + fr_idx);
		(*gl)->glNamedFramebufferDrawBuffer(to.id, GL_COLOR_ATTACHMENT0 + to_idx);
		(*gl)->glBlitNamedFramebuffer(fr.id, to.id, 0, 0, fw, fh, 0, 0, tw, th, GL_COLOR_BUFFER_BIT, GL_NEAREST);
	}
	...
	void FBO::CopyDepth(const FBO& fr, const FBO& to) {
		...
		GLuint fw = fr.width, fh = fr.height;
		GLuint tw = to.width, th = to.height;
		(*gl)->glBlitNamedFramebuffer(fr.id, to.id, 0, 0, fw, fh, 0, 0, tw, th, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
	}
	...
	void FBO::CopyStencil(const FBO& fr, const FBO& to) {
		...
		GLuint fw = fr.width, fh = fr.height;
		GLuint tw = to.width, th = to.height;
		(*gl)->glBlitNamedFramebuffer(fr.id, to.id, 0, 0, fw, fh, 0, 0, tw, th, GL_STENCIL_BUFFER_BIT, GL_NEAREST);
	}

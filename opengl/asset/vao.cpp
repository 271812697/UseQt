#include "pch.h"
#include "vao.h"


namespace asset {

    static GLuint curr_bound_vertex_array = 0;  // smart binding
    inline QOpenGLFunctions_4_5_Core** gl=getGlInstanceDPtr();
    VAO::VAO() : IAsset() {
        (*gl)->glCreateVertexArrays(1, &id);
    }

    VAO::~VAO() {
        Unbind();
        (*gl)->glDeleteVertexArrays(1, &id);
    }

    void VAO::Bind() const {
        if (id != curr_bound_vertex_array) {
            (*gl)->glBindVertexArray(id);
            curr_bound_vertex_array = id;
        }
    }

    void VAO::Unbind() const {
        if (curr_bound_vertex_array == id) {
            curr_bound_vertex_array = 0;
            (*gl)->glBindVertexArray(0);
        }
    }

    void VAO::SetVBO(GLuint vbo, GLuint attr_id, GLint offset, GLint size, GLint stride, GLenum type) const {
        (*gl)->glVertexArrayVertexBuffer(id, attr_id, vbo, offset, stride);
        (*gl)->glEnableVertexArrayAttrib(id, attr_id);
        (*gl)->glVertexArrayAttribBinding(id, attr_id, attr_id);

        switch (type) {
            case GL_HALF_FLOAT:
            case GL_FLOAT: {
                (*gl)->glVertexArrayAttribFormat(id, attr_id, size, type, GL_FALSE, 0); break;
            }
            case GL_UNSIGNED_INT:
            case GL_INT: {
                (*gl)->glVertexArrayAttribIFormat(id, attr_id, size, type, 0); break;  // notice the "I" here
            }
            case GL_DOUBLE: {
                (*gl)->glVertexArrayAttribLFormat(id, attr_id, size, type, 0); break;  // notice the "L" here
            }
            default: {
                 break;
            }
        }
    }

    void VAO::SetIBO(GLuint ibo) const {
        (*gl)->glVertexArrayElementBuffer(id, ibo);
    }

    void VAO::Draw(GLenum mode, GLsizei count) {
        Bind();
        glDrawElements(mode, count, GL_UNSIGNED_INT, 0);


        Unbind();  // with smart bindings, we never need to unbind after the draw call

    }

}

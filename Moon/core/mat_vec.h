...
namespace MOON {
	...
	Mat4<Scalar> perspective_matrix(Scalar fovy, Scalar aspect, Scalar zNear,
		Scalar zFar)
	{
		Scalar t = Scalar(zNear) * tan(fovy * Scalar(PI / 360.0));
		Scalar b = -t;
		Scalar l = b * aspect;
		Scalar r = t * aspect;
	...
	{
		Scalar t = zNear * tan(fovy * Scalar(PI / 360.0));
		Scalar b = -t;
		Scalar l = b * aspect;
		Scalar r = t * aspect;
	...
	{
		Scalar ca = cos(angle * Scalar(PI / 180.0));
		Scalar sa = sin(angle * Scalar(PI / 180.0));
	...
	{
		Mat4<Scalar> m(Scalar(0));
		Scalar a = angle * Scalar(PI / 180.0f);
		Scalar c = cosf(a);
		Scalar s = sinf(a);

	...
	bool update_matrices)
{
	...
	if (update_matrices)
	{
		...
		norm = (view * data.meshgl.model).inverse().transpose();
		...
		{
			float fH = tan(camera_view_angle / 360.0 * PI) * camera_dnear;
	...
	GLint moedli = glGetUniformLocation(shader_mesh, "model");
	GLint viewi = glGetUniformLocation(shader_mesh, "view");
	GLint proji = glGetUniformLocation(shader_mesh, "proj");
	GLint normi = glGetUniformLocation(shader_mesh, "normal_matrix");
	glUniformMatrix4fv(moedli, 1, GL_FALSE, data.meshgl.model.data());
	glUniformMatrix4fv(viewi, 1, GL_FALSE, view.data());
	glUniformMatrix4fv(proji, 1, GL_FALSE, proj.data());
	glUniformMatrix4fv(normi, 1, GL_FALSE, norm.data());

	// Light parameters
	GLint specular_exponenti = glGetUniformLocation(shader_mesh, "specular_exponent");
	GLint light_position_eyei = glGetUniformLocation(shader_mesh, "light_position_eye");
	GLint lighting_factori = glGetUniformLocation(shader_mesh, "lighting_factor");
	GLint fixed_colori = glGetUniformLocation(shader_mesh, "fixed_color");
	GLint texture_factori = glGetUniformLocation(shader_mesh, "texture_factor");
	GLint matcap_factori = glGetUniformLocation(shader_mesh, "matcap_factor");
	GLint double_sidedi = glGetUniformLocation(shader_mesh, "double_sided");
	...
	{
		glUniformMatrix4fv(glGetUniformLocation(shader_mesh, "shadow_view"), 1, GL_FALSE, shadow_view.data());
		glUniformMatrix4fv(glGetUniformLocation(shader_mesh, "shadow_proj"), 1, GL_FALSE, shadow_proj.data());
		glActiveTexture(GL_TEXTURE0 + 1);
		glBindTexture(GL_TEXTURE_2D, shadow_depth_tex);
		{
			glUniform1i(glGetUniformLocation(shader_mesh, "shadow_tex"), 1);
		}
	}
	glUniform1f(lighting_factori, lighting_factor); // enables lighting
	glUniform4f(fixed_colori, 0.0, 0.0, 0.0, 0.0);

	glUniform1i(glGetUniformLocation(shader_mesh, "is_directional_light"), eff_is_directional_light);
	glUniform1i(glGetUniformLocation(shader_mesh, "is_shadow_mapping"), is_shadow_mapping);
	glUniform1i(glGetUniformLocation(shader_mesh, "shadow_pass"), false);
	...
	for (int i = 0; i < data.meshgl.custom_shaders.size(); i++) {
		data.meshgl.draw_mesh_custom(i, *this);
	...
	if (is_set(data.show_overlay))
	{
		...
		if (data.lines.rows() > 0)
		{
			...
			viewi = glGetUniformLocation(shader_overlay_lines, "view");
			proji = glGetUniformLocation(shader_overlay_lines, "proj");
		...
		if (data.points.rows() > 0)
		{
			...
			viewi = glGetUniformLocation(shader_overlay_points, "view");
			proji = glGetUniformLocation(shader_overlay_points, "proj");
	...
	bool /*update_matrices*/)
{
	...
	glUniform1i(glGetUniformLocation(shader_mesh, "shadow_pass"), true);
	data.meshgl.draw_mesh(true);
	glUniform1i(glGetUniformLocation(shader_mesh, "shadow_pass"), false);
